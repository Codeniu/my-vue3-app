...
const showGuideLines = (canvas: fabric.Canvas) => {
  if (!canvas || !horizontalGuide || !verticalGuide || !showGuides.value) return
...
const handleObjectMoving = (e: any, canvas: fabric.Canvas) => {
  ...
  for (const obj of objects) {
    ...
    if (!snapHorizontal) {
      ...
      // 底部对齐
      else if (Math.abs(objBounds.bottom - targetBounds.bottom) < SNAP_THRESHOLD) {
      ...
      // 中心对齐
        ...
        Math.abs(
          objBounds.top + objBounds.height / 2 - (targetBounds.top + targetBounds.height / 2),
        ) < SNAP_THRESHOLD
      ) {
        movingObject.set({ top: targetBounds.top + targetBounds.height / 2 - objBounds.height / 2 })
        snapHorizontal = true
        ...
        if (horizontalGuide && showGuides.value) {
          const zoom = canvas.getZoom()
          const vpt = canvas.viewportTransform
          const lineY = (targetBounds.top + targetBounds.height / 2) * zoom + vpt[5]
    ...
    if (!snapVertical) {
      ...
      // 右侧对齐
      else if (Math.abs(objBounds.right - targetBounds.right) < SNAP_THRESHOLD) {
      ...
      // 中心对齐
        ...
        Math.abs(
          objBounds.left + objBounds.width / 2 - (targetBounds.left + targetBounds.width / 2),
        ) < SNAP_THRESHOLD
      ) {
        movingObject.set({ left: targetBounds.left + targetBounds.width / 2 - objBounds.width / 2 })
        snapVertical = true
        ...
        if (verticalGuide && showGuides.value) {
          const zoom = canvas.getZoom()
          const vpt = canvas.viewportTransform
          const lineX = (targetBounds.left + targetBounds.width / 2) * zoom + vpt[4]
